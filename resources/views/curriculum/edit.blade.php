...
@section('content')
    <!-- Resumo Init -->
        ...
        <div class='mb-5'>
            ...
            <form @if ($curriculum) action="{{ route('curriculums.update', $curriculum) }}" @else action="{{ route('curriculums.store') }}" @endif
            method="POST" x-on:submit.prevent="submitForm(event)" x-data="formController"   class="w-full">
                <div>
                    <x-curriculum.edit.first-step :curriculum="$curriculum" />
                </div>
                <div class="w-full">
